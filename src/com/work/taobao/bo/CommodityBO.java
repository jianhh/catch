...
import com.work.taobao.dao.CommodityDAO;
...
//		return this.commodityDAO.getShopTaobaoById(shopId);
//	}
	/**
	 * 查询店铺信息
	 * 
...
//	}
	...
	public boolean addGoodsExtendsProperty(GoodsExtendsPropertySchema schema)
		...
		return this.commodityDAO.addGoodsExtendsProperty(schema);
	}
	...
	public boolean addGoodsSkuList(GoodsSkuListSchema schema) throws Exception {
		return this.commodityDAO.addGoodsSkuList(schema);
	}
	 ...
	 * @throws Exception
	 */
	public boolean addGoodsSkuInfo(String shopId, GoodsSkuInfoSchema schema) throws Exception {
		List<GoodsSkuInfoSchema> schemaList = new ArrayList<GoodsSkuInfoSchema>();
		if(StringUtils.isNotEmpty(shopId)){
			schemaList.add(schema);
			this.modifyPirce(shopId, null, schemaList, null);
		}
		if(schemaList.size() > 0){
			return this.commodityDAO.addGoodsSkuInfo(schemaList.get(0));
		}else{
	 ...
	 * @throws Exception
	 */
	public boolean addGoodsPriceSection(String shopId, GoodsPriceSectionSchema schema)
			throws Exception {
		List<GoodsPriceSectionSchema> schemaList = new ArrayList<GoodsPriceSectionSchema>();
		if(StringUtils.isNotEmpty(shopId)){
			schemaList.add(schema);
			this.modifyPirce(shopId, null, null, schemaList);
		}
		if(schemaList.size() > 0){
			return this.commodityDAO.addGoodsPriceSection(schemaList.get(0));
		}else{
...
//	}
	...
	public boolean addShopTaobao(String shopId, String url) throws Exception {
		return this.commodityDAO.addShopTaobao(shopId, url);
	}
	...
	public boolean updateShopTaobao(ShopTaobaoSchema schema) throws Exception {
		return this.commodityDAO.updateShopTaobao(schema);
	}
...
//	}
	...
	public boolean addTaobaoCfgSysCategorySchema(TaobaoCfgSysCategorySchema schema)
			throws Exception {
		return this.commodityDAO.addTaobaoCfgSysCategorySchema(schema);
	}
	...
	public CfgSysCategorySchema getTaobaoCfgSysCategorySchemaByCatId(String catId)
			throws Exception {
		return this.commodityDAO.getTaobaoCfgSysCategorySchemaByCatId(catId);
	}
...
//		return this.commodityDAO.getShopTaobaoSchema();
//	}
		...
		if(shopCfgList != null){
			...
			if(schema != null){//修改商品表中的价格，即tb_goods
				if(StringUtils.isNotEmpty(schema.getN_buy_price())){
					...
					schema.setN_buy_price((int)Double.parseDouble(this.getPirce(schema.getN_buy_price(), keyValue))+ "");
				}
			...
			if(sectionList != null){//修改商品价格区间配置表中的价格，即tb_goods_price_section
				for(GoodsPriceSectionSchema section : sectionList){
					if(section != null){
						if(StringUtils.isNotEmpty(section.getN_price())){
