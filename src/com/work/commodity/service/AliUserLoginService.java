 ...
 * 
 */
public class AliUserLoginService extends BaseListener {

	private static JLogger logger = LoggerFactory
			.getLogger(AliUserLoginService.class);
	private static ConcurrentMap<String, ALiBaBa> tokens = new ConcurrentHashMap<String, ALiBaBa>();

	@Override
	public void doPerform(Req request, Resp response) throws Exception {
		if (logger.isDebugEnabled())
			logger.debug("AliUserLoginService......................");
		...
		String jsonStr = "";
		try {
			LogUtil.writeUserloginLog("AliUserLoginService:username："
					+ username + " userpw：" + userpw + " code:" + code
					+ " url:" + url);
			...
			if (StringUtils.isEmpty(username)) {
				...
				if (ali == null) {// 用户首次请求登录
					...
					ali = new ALiBaBa(username, userpw);
					state = ali.login("");// 登录
					if (state == 0) {
					...
					if (StringUtils.isNotEmpty(code)) {// 验证码校验
						...
						if (state == 0 && !userpw.equals(ali.passWord)) {// 用户第一次登录淘宝先提示要输入验证码，可能第二次用户输入验证码后把密码输入错了，这种情况要重新输入密码
							jsonStr = JsoupUtil.getLoginUrlJson(state
									.toString(), "密码错误", "", "");
						} else if (state == 0) {
							jsonStr = JsoupUtil.getLoginUrlJson(state
									.toString(), "登录成功", ali.sellerUrl, "");
							tokens.remove(username);
						}
						...
						if (url.toLowerCase().equals("http://me.1688.com")) {
							state = 10003;// 域名输入错误
							jsonStr = JsoupUtil.getLoginUrlJson(state
									.toString(), "地址输入错误，请重新输入", "", "");
						} else if (url.toLowerCase().equals(
								"http://domain.1688.com")) {
							state = 10003;// 域名输入错误
							jsonStr = JsoupUtil.getLoginUrlJson(state
									.toString(), "地址输入错误，请重新输入", "", "");
						} else {
							url += "/page/contactinfo.htm";
							Document doc = CommodityUtil.aliLoginDoc(url);// 获取阿里用户登录名的Doc
							String loginUserName = CommodityUtil
									.aliLoginUserName(doc, url);// 获取阿里用户登录名
							if (StringUtils.isEmpty(loginUserName)) {
								state = 10003;// 域名输入错误
								jsonStr = JsoupUtil.getLoginUrlJson(state
										.toString(), "地址输入错误，请重新输入", "", "");
							} else {
								if (loginUserName.toLowerCase().equals(
										username.toLowerCase())) {
									state = 0;// 成功
									String companyName = CommodityUtil
											.commodityName(doc, url);// 公司名
									String shopName = CommodityUtil
											.shopName(companyName);// 店铺名称
									jsonStr = JsoupUtil.getLoginUrlJson(state
											.toString(), "登录成功", url, shopName);
									tokens.remove(username);
								} else {
									state = 10004;// 域名和用户不匹配
									jsonStr = JsoupUtil.getLoginUrlJson(state
											.toString(), "域名和用户不匹配，请重新输入", "",
											"");
								}
							}
						}
			...
			logger.error("用户登录失败：username：" + username + " userpw：" + userpw
					+ " code:" + code, e);
			jsonStr = JsoupUtil.getJson("10000", "网络异常，请稍后再试");
			// 发送邮件
			SimpleMailSender sms = MailSenderFactory.getSender();
			sms.send(CommodityContent.getRecipients(), "用户登录失败：username："
					+ username + " userpw：" + userpw + " code:" + code, LogUtil
					.getExceptionError(e));
		}
		LogUtil.writeUserloginLog("用户登录响应:" + username + jsonStr.toString());
		int end = (int) System.currentTimeMillis(); // 结束时间
		int re = end - start; // 处理时间
		LogUtil.writeCommodityLog("AliUserLoginService:" + username
				+ "用户登录共耗时: " + re + "毫秒");
	...
	public static void main(String[] args) {
		Integer shopId = 16244336;
		System.out.println(shopId.intValue() == 16244336);
	}
}
